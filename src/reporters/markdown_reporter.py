...
class MarkdownReporter:
    ...
    def _write_key_findings(self, f, summary: Dict):
        ...
        if critical_count > 0:
            f.write(f"🔴 **CRITICAL:** {critical_count} critical security/governance issue(s) found\n\n")
        
        if high_count > 0:
            f.write(f"🟠 **HIGH:** {high_count} high-severity issue(s) require attention\n\n")
        
        if medium_count > 0:
            f.write(f"🟡 **MEDIUM:** {medium_count} medium-severity issue(s) identified\n\n")
        
        # Specific findings
        stats = summary.get('statistics', {})
        
        if stats.get('accounts_with_public_access', 0) > 0:
            f.write(f"⚠️ {stats.get('accounts_with_public_access', 0)} storage account(s) allow public blob access\n\n")
        
        if stats.get('accounts_without_https_only', 0) > 0:
            f.write(f"⚠️ {stats.get('accounts_without_https_only', 0)} storage account(s) do not enforce HTTPS-only\n\n")
        
        if stats.get('accounts_without_soft_delete', 0) > 0:
            f.write(f"⚠️ {stats.get('accounts_without_soft_delete', 0)} storage account(s) do not have soft delete enabled\n\n")
        
        stale_size_gb = stats.get('total_stale_size_bytes', 0) / (1024 ** 3)
        if stale_size_gb > 1:
            f.write(f"💾 {format_bytes(stats.get('total_stale_size_bytes', 0))} of stale data detected (90+ days no access)\n\n")
